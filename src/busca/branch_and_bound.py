...
def find_neighbor(adjacent_vertex_list: List[AdjacentVertex], vertex_list_to_analyze, best_so_far: float):
    new_list = []
    for vertex in adjacent_vertex_list:
        if vertex.Vertex not in vertex_list_to_analyze[0]:
            new_vertex_in_list = new_item_of_list(vertex_list_to_analyze[0].copy(), vertex_list_to_analyze[1], vertex, best_so_far)
            if len(new_vertex_in_list) > 0:
                new_list.append(new_vertex_in_list)
    return new_list


def new_item_of_list(node_list: List[int], cost: float, new_vertex: AdjacentVertex, best_so_far: float):
